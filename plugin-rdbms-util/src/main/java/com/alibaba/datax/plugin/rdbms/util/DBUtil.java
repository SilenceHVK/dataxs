...
//                    throw new Exception(DBUtilErrorCode.JDBC_NULL.toString());
	 ...
	 * @return
	 */
	public static String chooseJdbcUrlWithoutRetry(final DataBaseType dataBaseType, final String jdbcUrl, final String username,
																								 final String password, final List<String> preSql, final boolean checkSlave) {
		if (null == jdbcUrl || jdbcUrl.isEmpty()) {
			throw DataXException.asDataXException(
				DBUtilErrorCode.CONF_ERROR,
				String.format("您的jdbcUrl的配置信息有错, 因为jdbcUrl[%s]不能为空. 请检查您的配置并作出修改.",
					StringUtils.join(jdbcUrl, ",")));
		}
		boolean connOK = false;
		...
		throw DataXException.asDataXException(
			DBUtilErrorCode.CONN_DB_ERROR,
			String.format("数据库连接失败. 因为根据您配置的连接信息,无法从:%s 中找到可连接的jdbcUrl. 请检查您的配置并作出修改.", jdbcUrl));
	}
	 ...
	 * @return
	 */
	public static String chooseJdbcUrl(final DataBaseType dataBaseType, final String jdbcUrl, final String username,
																		 final String password, final List<String> preSql, final boolean checkSlave) {
		if (null == jdbcUrl || jdbcUrl.isEmpty()) {
			throw DataXException.asDataXException(
				DBUtilErrorCode.CONF_ERROR,
				String.format("您的jdbcUrl的配置信息有错, 因为jdbcUrl[%s]不能为空. 请检查您的配置并作出修改.", jdbcUrl));
		}
		...
		try {
			...
			throw DataXException.asDataXException(
				DBUtilErrorCode.CONN_DB_ERROR,
				String.format("数据库连接失败. 因为根据您配置的连接信息,无法从:%s 中找到可连接的jdbcUrl. 请检查您的配置并作出修改.", jdbcUrl), e);
		}
	}
	...
	public static boolean checkInsertPrivilege(DataBaseType dataBaseType, String jdbcURL, String userName, String password, List<JSONObject> tableList) {
		Connection connection = connect(dataBaseType, jdbcURL, userName, password);
		...
		for (JSONObject jsonObject : tableList) {
			String tableName = jsonObject.getString(Key.TABLE_NAME);
	...
	public static boolean checkDeletePrivilege(DataBaseType dataBaseType, String jdbcURL, String userName, String password, List<JSONObject> tableList) {
		Connection connection = connect(dataBaseType, jdbcURL, userName, password);
	...
	public static Connection getConnectDuckDB(DataBaseType dataBaseType, final String jdbcUrl) {
		try {
