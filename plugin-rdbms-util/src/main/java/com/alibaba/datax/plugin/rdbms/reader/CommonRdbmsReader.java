...
public class CommonRdbmsReader {
	...
	public static class Job {
		...
		public void preCheck(Configuration originalConfig, DataBaseType dataBaseType) {
			...
			// 声明线程池
			ExecutorService exec = new ThreadPoolExecutor(conns.size(),
				conns.size() << 1, 60, TimeUnit.SECONDS, new LinkedBlockingDeque<>(),
			...
			for (Object conn : conns) {
				...
				PreCheckTask task = new PreCheckTask(username, password, connConf, dataBaseType, splitPK);
				taskList.add(task);
			}
	...
	public static class Task {
		...
		public void init(Configuration readerSliceConfig) {
			...
			if (this.jdbcUrl.startsWith(com.alibaba.datax.plugin.rdbms.writer.Constant.OB10_SPLIT_STRING)
				...
				String[] ss =
					this.jdbcUrl.split(
						com.alibaba.datax.plugin.rdbms.writer.Constant.OB10_SPLIT_STRING_PATTERN);
				if (ss.length != 3) {
...
//      queryPerfRecord.start();
			...
			Connection conn = Objects.equals(this.dataBaseType, DataBaseType.DUCKDB) ? DBUtil.getConnectDuckDB(this.dataBaseType, jdbcUrl) :
				DBUtil.getConnection(this.dataBaseType, jdbcUrl, username, password);
...
//        queryPerfRecord.end();
				...
				PerfRecord allResultPerfRecord =
					new PerfRecord(taskGroupId, taskId, PerfRecord.PHASE.RESULT_NEXT_ALL);
				...
				while (rs.next()) {
					...
					this.transportOneRecord(
						recordSender, rs, metaData, columnNumber, mandatoryEncoding, taskPluginCollector);
		...
		protected Record transportOneRecord(
			RecordSender recordSender,
			ResultSet rs,
					...
					switch (metaData.getColumnType(i)) {
						...
						case Types.LONGNVARCHAR:
							...
							if (StringUtils.isBlank(mandatoryEncoding)) {
								...
								rawData =
									new String(
										(rs.getBytes(i) == null ? EMPTY_CHAR_ARRAY : rs.getBytes(i)),
										mandatoryEncoding);
							}
