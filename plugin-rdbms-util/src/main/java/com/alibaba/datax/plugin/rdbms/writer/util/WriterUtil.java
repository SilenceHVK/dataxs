...
public final class WriterUtil {
    ...
    public static List<Configuration> doSplit(Configuration simplifiedConf, int adviceNumber) {
        ...
        for (int i = 0, len = conns.size(); i < len; i++) {
            ...
            List<JSONObject> tables = sliceConfig.getList(com.alibaba.datax.plugin.rdbms.reader.Key.TABLE, JSONObject.class);
            Validate.isTrue(null != tables && !tables.isEmpty(), "您读取数据库表配置错误.");

            // 对 table 拆分
            for(JSONObject table : tables) {
                String tableName = table.getString(com.alibaba.datax.plugin.rdbms.reader.Key.TABLE_NAME);
                List<String> columns = JSONObject.parseArray(table.getString(com.alibaba.datax.plugin.rdbms.reader.Key.COLUMN), String.class);

                Configuration tempSlice = sliceConfig.clone();
                tempSlice.set(Key.TABLE, tableName);
                tempSlice.set(Key.COLUMN, columns);
                tempSlice.set(Constant.INSERT_OR_REPLACE_TEMPLATE_MARK, table.getString(Constant.INSERT_OR_REPLACE_TEMPLATE_MARK));
                tempSlice.set(Key.PRE_SQL, renderPreOrPostSqls(preSqls, tableName));
                tempSlice.set(Key.POST_SQL, renderPreOrPostSqls(postSqls, tableName));
